...
class GoogleNanoImageProvider(ImageProviderBase):
    ...
    async def generate(
        ...
        **kwargs: Any
        ...
        if not self.api_key:
            # For google and googlevertex, allow empty API key (use local service)
            if provider == "googlevertex" or provider == "google":
                print(f"Using {provider} without API key: {self.base_url}")
            else:
...
GoogleNanoProvider = GoogleNanoImageProvider
